...
public class StudentController {
    ...
    public String studentSearch(Model m, @RequestParam("freeText") String freeText) throws StudentException {
      //  Integer userId = (Integer) session.getAttribute("userId");
		ArrayList<Student> studList = (ArrayList<Student>) studentService.findStudent(freeText);
		/*
		 * for (Student student : studList) { System.out.println(student);
		 * 
		 * }
		 */
		studList.forEach(s -> System.out.println(s.getId()));
        m.addAttribute("command",studList );
